...
class Vigenere(object):
	...
	def __check_valid_input(self, input):		
		for char in input:
			count = self.__unwanted_chars.count(char)
			if count > 0:
				raise Exception('Message only allows alphanumeric characters.')
	...
	def __construct_mixed_alphabet(self, key):
		...
		while len(mixed) < len(self.alphabet) and passes <= max_passes:
			for char in string:
	...
	def __construct_table(self, alphabet):
		...
		mat = {}
		row = ''
		for r_symbol in alphabet:
			for c_symbol in alphabet:
				row += alphabet[c]				
				c += 1
				if c >= dim: c = 0
			mat[self.__alphabet[r]] = row
			row = ''
			r += 1
			c = r % dim
		return mat
